...
function BirdHouse(params) {
	...
	function save_access_token() {
		...
		var config = {
			...
			user_id: cfg.user_id // aanpassing
	...
	function tweet(text, callback) {
		...
		if (authorized === false) {
			...
			var winBG = Titanium.UI.createWindow({
				...
				// De applicatie heeft geen nut zonder locatie. Daarom kan er pas een tweet verzonden worden als de locatie gevonden is.
				Ti.Geolocation.purpose="test";
				Titanium.Geolocation.getCurrentPosition(function(e)
				{
				    if (e.error)
				    {
				        alert('Sorry, we kunnen je huidige locatie niet vinden.');
				        return;
				    }
				    ...
				    // Hier genereren we de wild tweet
					send_tweet("status="+escape(text)+"&lat="+latitude+"&long="+ longitude + "&display_coordinates=true",function(retval){
...
Ti.API.info('shortlink '+shorturl);
	...
	function get_tweets(params,callback) {
		...
		api("https://api.twitter.com/1/statuses/friends_timeline.json","GET",'source=instagram&include_entities=true',function(tweets){
			try {
